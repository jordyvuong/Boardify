  ...
  <div class="edit-profile-container">
    ...
    <div class="profile-photo-container">
      <div class="profile-photo-wrapper">
        <img v-if="previewPhoto" :src="previewPhoto" alt="Photo de profil" class="profile-photo" />
...
<script>
// filepath: c:\Users\kaize\Documents\B2\Hackaton\Boardify\Boardify\src\views\EditProfileView.vue
...
export default {
  setup() {
    ...
    const initialDisplayName = ref(authStore.user?.displayName || '')
    const initialPhotoURL = ref(authStore.user?.photoURL || '')
    ...
    const hasChanges = computed(() => {
      const displayNameChanged = displayName.value !== initialDisplayName.value
      const photoChanged = photoFile.value !== null
      const passwordChanged = showPasswordFields.value && newPassword.value && currentPassword.value && newPassword.value === confirmPassword.value
      return displayNameChanged || photoChanged || passwordChanged
    })
    ...
    const handleFileChange = (event) => {
      ...
      if (file && file.type.startsWith('image/')) {
        ...
        previewPhoto.value = initialPhotoURL.value
    ...
    const saveProfile = async () => {
      try {
        ...
        if (displayName.value !== initialDisplayName.value) {
        ...
        if (updates.photoURL) {
          initialPhotoURL.value = updates.photoURL
        }
    ...
    onMounted(() => {
      ...
      initialDisplayName.value = authStore.user?.displayName || ''
      initialPhotoURL.value = authStore.user?.photoURL || ''
    })
